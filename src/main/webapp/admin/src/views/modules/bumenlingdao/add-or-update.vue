    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="部门" prop="bumen">
          <el-select :disabled="ro.bumen" v-model="ruleForm.bumen" placeholder="请选择部门">
            <el-option
...
export default {
  data() {
	...
	lianxidianhua : false,
      ...
      rules: {
          lingdaozhanghao: [
                { required: true, message: '领导账号不能为空', trigger: 'blur' },
          ],
          mima: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `bumenlingdao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.bumenlingdaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
