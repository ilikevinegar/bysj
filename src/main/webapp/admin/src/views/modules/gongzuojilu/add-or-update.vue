    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="任务名称" prop="renwumingcheng">
          <el-input v-model="ruleForm.renwumingcheng" 
              placeholder="任务名称" clearable  :readonly="ro.renwumingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="任务名称" prop="renwumingcheng">
              <el-input v-model="ruleForm.renwumingcheng" 
                placeholder="任务名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.gongzuofujian" label="工作附件" prop="gongzuofujian">
          <file-upload
          tip="点击上传工作附件"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.gongzuofujian?ruleForm.gongzuofujian:''"
          @change="gongzuofujianUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.gongzuofujian" label="工作附件" prop="gongzuofujian">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.gongzuofujian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="提交内容" prop="tijiaoneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="提交内容"
                  v-model="ruleForm.tijiaoneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tijiaoneirong" label="提交内容" prop="tijiaoneirong">
                    <span>{{ruleForm.tijiaoneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.renwumingcheng = true;
          ...
          if(o=='gongzuofujian'){
            this.ruleForm.gongzuofujian = obj[o];
	    this.ro.gongzuofujian = true;
            continue;
          }
          if(o=='tijiaoneirong'){
            this.ruleForm.tijiaoneirong = obj[o];
	    this.ro.tijiaoneirong = true;
            continue;
          }
          if(o=='tijiaoriqi'){
	...
	if(this.ruleForm.gongzuofujian!=null) {
		this.ruleForm.gongzuofujian = this.ruleForm.gongzuofujian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `gongzuojilu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.gongzuojiluCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    gongzuofujianUploadChange(fileUrls) {
	this.ruleForm.gongzuofujian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
