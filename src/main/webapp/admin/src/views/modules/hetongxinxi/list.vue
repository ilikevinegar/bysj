    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                <el-form-item :label="contents.inputTitle == 1 ? '合同编号' : ''">
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 1" prefix-icon="el-icon-search" v-model="searchForm.hetongbianhao" placeholder="合同编号" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search" v-model="searchForm.hetongbianhao" placeholder="合同编号" clearable></el-input>
                  <el-input v-if="contents.inputIcon == 0" v-model="searchForm.hetongbianhao" placeholder="合同编号" clearable></el-input>
                </el-form-item>
		<el-form-item class="select" label="合同类型" prop="hetongleixing">
		  <el-select  @change="hetongleixingChange" clearable v-model="searchForm.hetongleixing" placeholder="请选择合同类型">
		    <el-option
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            ...
            <el-button
              v-if="isAuth('hetongxinxi','打印') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="success"
              icon="el-icon-printer"
              @click="printJson"
            >{{ contents.btnAdAllFont == 1?'打印':'' }}</el-button>
            <el-button
              v-if="isAuth('hetongxinxi','打印') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success"
              @click="printJson"
            >{{ contents.btnAdAllFont == 1?'打印':'' }}<i class="el-icon-printer el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('hetongxinxi','打印') && contents.btnAdAllIcon == 0"
              type="success"
              @click="printJson"
            >{{ contents.btnAdAllFont == 1?'打印':'' }}</el-button>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="合同类型">
		     <template slot-scope="scope">
                       {{scope.row.hetongleixing}}
                     </template>
                </el-table-column>
                  <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"  prop="hetongwenjian"
                   :header-align="contents.tableAlign"
                    label="合同文件">
                    <template slot-scope="scope">
                      <el-button type="text" size="small" @click="download($base.url+scope.row.hetongwenjian)">下载</el-button>
                    </template>
                  </el-table-column>
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign" 
  ...
  methods: {
    ...
    getDataList() {
      ...
      let params = {
        ...
        sort: 'id',
          ...
          if(this.searchForm.hetongleixing!='' && this.searchForm.hetongleixing!=undefined){
            params['hetongleixing'] = '%' + this.searchForm.hetongleixing + '%'
          }
    ...
    async printJson() {
      ...
      for (let i=0; i < this.list.length; i++) {
          data.push({
          hetongbianhao: this.list[i].hetongbianhao,
          hetongmingcheng: this.list[i].hetongmingcheng,
          hetongleixing: this.list[i].hetongleixing,
          yuangonggonghao: this.list[i].yuangonggonghao,
          yuangongxingming: this.list[i].yuangongxingming,
          bumen: this.list[i].bumen,
          zhiwei: this.list[i].zhiwei,
          riqi: this.list[i].riqi,
          hetongxinxi: this.list[i].hetongxinxi,
          lingdaozhanghao: this.list[i].lingdaozhanghao,
          lingdaoxingming: this.list[i].lingdaoxingming,
        })
      }
      printJS({
        printable: data,
	  ...
	  {
		field: 'lingdaozhanghao',
		displayName: '领导账号',
		columnSize: 1
	  },
	  {
		field: 'lingdaoxingming',
		displayName: '领导姓名',
		columnSize: 1
	  },
        ],
